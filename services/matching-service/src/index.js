...
const calculateMatchScore = (u1, u2) => {
  ...
  if (u1.knowledgeLevel && u2.knowledgeLevel && sharedCourses.length > 0) {
    const diff = Math.abs(u1.knowledgeLevel - u2.knowledgeLevel);
    if (diff <= 20) {
      score += 10;
      reasons.push(`Similar knowledge level (${u1.knowledgeLevel}% vs ${u2.knowledgeLevel}%)`);
    } else if (diff <= 40) {
      score += 5;
      reasons.push(`Close knowledge level`);
    }
  }
...
const runMatching = async (userId) => {
  ...
  for (const m of topMatches) {
    ...
    } else if (existing.score !== m.score) {
...
const start = async () => {
  ...
  await kafkaConsumer.subscribe({ topic: TOPICS.DELETE_MATCH });
  ...
  await kafkaConsumer.run({
    eachMessage: async ({ message }) => {
      ...
      try {
        ...
        if (eventName === TOPICS.PREFERENCES_UPDATED) {
          await prisma.userSnapshot.upsert({
            ...
            update: {
              ...
              knowledgeLevel: payload.knowledgeLevel || null
            },
            create: payload
          });
        ...
        // -------- DELETE MATCH (dismiss) --------
        if (eventName === TOPICS.DELETE_MATCH) {
