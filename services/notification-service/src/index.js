...
const handleEvent = async (eventName, payload) => {
  try {
    ...
    // -------- BUDDY REQUEST SENT --------
    if (eventName === TOPICS.BUDDY_REQUEST_SENT) {
      ...
      await prisma.notification.create({
        data: {
          ...
          body: `You received a new buddy request`
        }
      });
    }

    // -------- BUDDY REQUEST ACCEPTED --------
    if (eventName === TOPICS.BUDDY_REQUEST_ACCEPTED) {
      const { senderId, receiverId } = payload;
      ...
      await prisma.notification.create({
        data: {
          ...
          body: `Your buddy request was accepted`
        }
      });
    }
...
const start = async () => {
  ...
  await kafkaConsumer.subscribe({ topic: TOPICS.BUDDY_REQUEST_SENT });
  await kafkaConsumer.subscribe({ topic: TOPICS.BUDDY_REQUEST_ACCEPTED });
        ...
        try {
          ...
          if (!notification) {
            return await kafkaProducer.send({
              topic: replyTo,
              messages: [{ value: JSON.stringify({ correlationId, payload: { success: false, error: 'Notification not found' } }) }]
            });
          }
          ...
          await kafkaProducer.send({
            topic: replyTo,
            messages: [{ value: JSON.stringify({ correlationId, payload: { success: true, data: { notification: updated } } }) }]
          });
        } catch (err) {
          await kafkaProducer.send({
            topic: replyTo,
