...
const STUDY_STYLE = ['NOTES', 'LISTENING', 'DISCUSSING', 'QUIET', 'PROBLEM_SOLVING', 'OTHER'];

const STUDY_PACE_MAP = {
  'Slow & Thorough': 'SLOW',
  'Moderate': 'MEDIUM',
  'Fast-paced': 'FAST'
};
    ...
    eachMessage: async ({ message }) => {
      ...
      try {
        ...
        if (eventName === TOPICS.UPDATE_PROFILE) {
          ...
          // map human-readable → enum
          studyPace = STUDY_PACE_MAP[studyPace] || normalizeEnum(studyPace);
          studyMode = STUDY_MODE_MAP[studyMode] || normalizeEnum(studyMode);
          studyStyle = STUDY_STYLE_MAP[studyStyle] || normalizeEnum(studyStyle);
