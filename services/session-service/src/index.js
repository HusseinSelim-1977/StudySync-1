...
const start = async () => {
  ...
  const topics = [
    ...
    TOPICS.CANCEL_SESSION,
    TOPICS.UPDATE_SESSION,
  ];
      ...
      try {
        ...
        if (eventName === TOPICS.CREATE_SESSION) {
          const {
            ...
            sessionType, meetingLink, location, maxParticipants, invitedUserIds, knowledgeLevel
          } = payload;
        ...
        // ---------------- CANCEL SESSION ----------------
        if (eventName === TOPICS.CANCEL_SESSION) {
          const { sessionId, userId } = payload;
          ...
          if (session.creatorId !== userId) {
            return sendResponse(replyTo, correlationId, {
              success: false, error: 'Only the creator can cancel this session'
            });
          }

          if (session.status === 'CANCELLED') {
            return sendResponse(replyTo, correlationId, {
              success: false, error: 'Session already cancelled'
            });
          }
          ...
          await kafkaProducer.send({
            ...
            messages: [{
              value: JSON.stringify(
                formatMessage(TOPICS.SESSION_CANCELLED, 'session-service', {
                  sessionId,
                  creatorId: session.creatorId
                })
              )
        ...
        // ---------------- UPDATE SESSION ----------------
        if (eventName === TOPICS.UPDATE_SESSION) {
          const { sessionId, userId, title, topic, dateTime, duration, meetingLink, location, invitedUserIds, removeUserIds, addUserIds } = payload;
          ...
          if (session.creatorId !== userId) {
            return sendResponse(replyTo, correlationId, {
              success: false, error: 'Only the creator can update this session'
            });
          }
          ...
          if (removeUserIds && Array.isArray(removeUserIds)) {
            await prisma.sessionParticipant.deleteMany({
              where: { sessionId, userId: { in: removeUserIds } }
            });
          }
