...
const handleMessage = async (parsed) => {
  ...
  try {
    ...
    if (eventName === TOPICS.REMOVE_BUDDY) {
      const { userId, contactUserId } = payload || {};

      if (!userId || !contactUserId) {
        return sendResponse(replyTo, correlationId, {
          success: false, error: 'userId and contactUserId required'
        });
      }
    ...
    // -------- DELETE USER --------
    if (eventName === TOPICS.DELETE_USER) {
      const { userId } = payload || {};
    ...
    // -------- SEARCH USERS --------
    if (eventName === TOPICS.SEARCH_USERS) {
      const { query } = payload || {};
